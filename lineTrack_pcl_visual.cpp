...
/**
 * 这个是用pcl的窗口来显示固定图片的，是最原始的可视化，后面采用的rviz
 *
 *
 * */
...
/**测试三：二值图像中标出轨迹，同时用pcl可视化左右边线和均值轨迹**/
void test1(){
...
/**在test2的基础上加了判断，当两边线均存在时才画出轨迹**/
void test3(){
    /** 1.get the picture **/
    cv::VideoCapture cap  = cv::VideoCapture("../dataset/test.mp4");
    while (cap.isOpened()){
        cv::Mat frame;
        cap.read(frame);
    ...
    //    cv::erode(binary,binary,cv::Mat(),cv::Point(-1,-1),2);
        ...
        cout<<uvLeft.size()<<endl;
        cout<<uvRight.size()<<endl;
...
//            pcl::visualization::PCLVisualizer::Ptr viewer = new pcl::visualization::PCLVisualizer("line");
